...
describe('Event → WorkflowEngine → AgentRuntime Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const triggerHandler: EventTriggerHandler = async (workflow, event) => {
      return await workflowEngine.executeWorkflow(workflow, event);
    };
  ...
  describe('Complete Event-to-Agent Execution Flow', () => {
    it('should execute complete flow from manual event trigger to agent execution', async () => {
      ...
      // 2. Create agents in runtime
      const rootAgentId = agentRuntime.createAgent(rootAgentConfig);
      const level1AgentId = agentRuntime.createAgent(level1AgentConfig);

      expect(rootAgentId).toBe('integration-root-agent');
      expect(level1AgentId).toBe('integration-level1-agent');

      // 3. Set up workflow configuration
      ...
      // 4. Register workflow with event system
      await eventSystem.registerWorkflow(workflowConfig);

      // 5. Verify workflow was registered
      ...
      // 8. Check execution status using the workflowExecutionId
    ...
    it('should handle multi-level workflow execution through event system', async () => {
      ...
      if (mathEventResult.workflowExecutionId) {
        ...
        expect(completedMathExecution?.workflow.levels).toHaveLength(1);
